...
`calculateMatchPct` 함수는 사용자와 팀의 시간대, 페이스, 목표거리, 요일(월~일) 7개 벡터 중 시간대·요일을 비교합니다. 시간대가 다르면 40%, 활동 요일이 다를수록 최대 60%의 가중치가 더해져, **서로 다를수록 매칭률이 높게(70~99% 범위) 나오도록** 설계되어 있습니다. 
